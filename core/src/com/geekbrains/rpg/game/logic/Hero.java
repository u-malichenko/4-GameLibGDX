...
//        batch.draw(textures[0][getCurrentFrameIndex()], position.x - 30, position.y - 15, 30, 30, 60, 60, 1, 1, 0);
//        //position.y - 15 - нижний левый край смещаем на 15 вниз
//        if (hp < hpMax) {
...
//    }
    ...
    public void update(float dt) {
        ...
        if (Gdx.input.isButtonJustPressed(Input.Buttons.LEFT)) {
            ...
            dst.set(gc.getMouse()); // подставляем координаты мышки полученные там на каждом кадре
            state = State.MOVE;
            target = null;
            sleepTimer = -3;
           //TODO stateTimer = 0;
